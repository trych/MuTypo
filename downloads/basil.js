...
/**
 * @summary Measures an object, text or page.
 * @description TODO.
 *
 * @cat     Document
 * @subcat  Page Items
 * @method  measure
 *
 * @param   {PageItem} item The page item, text, page or spread to measure.
 * @param   {String} [type] The property to measure.
 * @param   {Object} [settings] A settings object to control the function's behavior.
 * @param   {Boolean} [settings.boundingBox] Width, height and size relate to bounding box. Default: TODO
 * @param   {Boolean} [settings.textParts] TODO
 *
 * @return  {Number|Object} The measured value as a number or as an object with key value pairs.
 */
pub.measure = function(item, type, settings) {
  var m = measureItem(item, type, settings);
  currMeasurePoints = null;
  return m;
};

// private function
var measureItem = function(item, type, settings) {
  ...
  // set settings object to given values or defaults
  settings.boundingBox = settings.hasOwnProperty("boundingBox") ? settings.boundingBox : false;
  settings.textParts = settings.hasOwnProperty("textParts") ? settings.textParts : false;
  ...
  if(isArray(type)) {
    ...
    for (var i = 0; i < type.length; i++) {
      if(!isString(type[i])) {
        error("measure(), invalid type parameter. If an array of measurement types is given as argument, these types must be provided as strings.");
      }
      measurements[type[i]] = measureItem(item, type[i], settings);
    }
    return measurements;
  }

  var legalTypes = ["width", "height", "size", "left", "right", "top", "bottom", "x", "y", "position"];

  if(isText(item)) {

    legalTypes = legalTypes.concat(["rotation", "baseline", "xHeight", "capHeight", "ascent", "descent"]);

    if(!contains(legalTypes, type)) {
      error("measure(), invalid type parameter \"" + type + "\"!\rIf measuring a text object use any of the following types: " + legalTypes.join(", ") + " or an array containing any of these types.");
    }
    ...
    if (type === "ascent" || type === "descent") {
    ...
    } else if (type === "xHeight" || type === "capHeight") {
      var itemFont = item.appliedFont.postscriptName;

      if(typeMetrics.hasOwnProperty(itemFont)) {
        // type has been measured previously, use measured values
        return typeMetrics[itemFont][type] / 12 * item.pointSize * (item.verticalScale / 100);
      }

      return getTypeMetrics(item)[type] / 12 * item.pointSize * (item.verticalScale / 100);

    } else {
      // all other types require actual measuring of the text on the page
      // check if text is in a text frame and can be measured at all
      if(!item.parentTextFrames.length) {
        warning("measure(), cannot measure " + type + " of " + item.constructor.name.toLowerCase() + ", as it is placed in the text overflow area. Null is returned instead.");
      ...
      if (type === "baseline") {
        return item.baseline - currOriginY;
      }

      var parentFrame = item.parentTextFrames[0];

      if(type === "rotation") {
        return -parentFrame.rotationAngle;
      }

      // all other types require getting the text bounds

      var cp; // corner points of the text bounds
      if(currMeasurePoints) {
        // recursive call, points have been measured in a previous call
        cp = currMeasurePoints;

      } else {

        cp = {tl: {}, tr: {}, bl: {}, br: {}};
        var theta = -parentFrame.rotationAngle * PI / 180;

        if(item.lines.length === 1 ||
           item instanceof Word /* and text parts is off TODO*/) {
          // text does not break line

          var ex, ey;
          var sx = item.horizontalOffset - currOriginX;
          var sy = item.baseline - currOriginY;
          if(item instanceof Line ||
            (item instanceof Word && item.lines.length > 1)) {

            var le = getLineEnd(item.lines.firstItem(), theta);
            ex = le.x - currOriginX;
            ey = le.y - currOriginY;

          } else {
            ex = item.endHorizontalOffset - currOriginX;
            ey = item.endBaseline - currOriginY;
          }

          cp.tl.x = sx - sin(theta) * -item.ascent;
          cp.tl.y = sy + cos(theta) * -item.ascent;
          cp.bl.x = sx - sin(theta) * item.descent;
          cp.bl.y = sy + cos(theta) * item.descent;
          cp.tr.x = ex - sin(theta) * -item.ascent;
          cp.tr.y = ey + cos(theta) * -item.ascent;
          cp.br.x = ex - sin(theta) * item.descent;
          cp.br.y = ey + cos(theta) * item.descent;
        } else {
          // multi-line text

          var lastChar;
          if (item instanceof Word /* && text array is on TODO*/) {
            // word breaks line, find end of line
            lastChar = item.lines.firstItem().characters.lastItem();

            // TODO

          } else {
            // multiline text that spans the width of a text column

            var firstColumn = item.textColumns.firstItem();
            var firstLine = item.lines.firstItem();
            var lastLine;

            if(item.textColumns.length > 1) {
              // text breaks column, find end of column
              lastLine = firstColumn.lines.lastItem();
            } else {
              lastLine = item.lines.lastItem();
            }

            // get corner points of parent text frame
            cp = getCornerPoints(parentFrame, unitToEnum(currUnits));

            // find smallest leftOffset and rightOffset
            var leftOffset = Infinity;
            var rightOffset = Infinity;

            for (var i = 0; i < item.lines.length; i++) {
              var line = item.lines[i];

              if(leftOffset > 0) {
                var lx = line.horizontalOffset
                var ly = line.baseline;

                var leftDist = distPointLine(lx, ly, cp.tl.x, cp.tl.y, cp.bl.x, cp.bl.y);

                if(leftDist < leftOffset) {
                  leftOffset = leftDist;
                }
              }

              if(rightOffset > 0) {
                var le = getLineEnd(line, theta);
                var rx = le.x;
                var ry = le.y;

                var rightDist = -distPointLine(rx, ry, cp.tr.x, cp.tr.y, cp.br.x, cp.br.y);

                if(rightDist < rightOffset) {
                  rightOffset = rightDist;
                }
              }

              if(line === lastLine || leftOffset + rightOffset === 0) {
                break;
              }
            }

            // calculate offset from first/last baseline to top and bottom of the text column
            var topOffset = distPointLine(firstLine.horizontalOffset, firstLine.baseline, cp.tl.x, cp.tl.y, cp.tr.x, cp.tr.y) + firstLine.ascent * (firstLine.verticalScale / 100) * (parentFrame.verticalScale / 100);
            var bottomOffset = distPointLine(lastLine.horizontalOffset, lastLine.baseline, cp.bl.x, cp.bl.y, cp.br.x, cp.br.y) - lastLine.descent * (lastLine.verticalScale / 100) * (parentFrame.verticalScale / 100);


            // shift the virtual text column bounding box
            // to be become the text bounding box we're looking for
            cp.tl.x += cos(theta) * leftOffset - sin(theta) * -topOffset - currOriginX;
            cp.tl.y += sin(theta) * leftOffset + cos(theta) * -topOffset - currOriginY;

            cp.bl.x += cos(theta) * leftOffset - sin(theta) * -bottomOffset - currOriginX;
            cp.bl.y += sin(theta) * leftOffset + cos(theta) * -bottomOffset - currOriginY;

            cp.tr.x += cos(theta) * -rightOffset - sin(theta) * -topOffset - currOriginX
            cp.tr.y += sin(theta) * -rightOffset + cos(theta) * -topOffset - currOriginY;

            cp.br.x += cos(theta) * -rightOffset - sin(theta) * -bottomOffset - currOriginX
            cp.br.y += sin(theta) * -rightOffset + cos(theta) * -bottomOffset - currOriginY;

          }

        }

        currMeasurePoints = cp;

      }


      if(type === "width") {
        return dist(cp.tl.x, cp.tl.y, cp.tr.x, cp.tr.y);

      } else if (type === "height") {
        return dist(cp.tl.x, cp.tl.y, cp.bl.x, cp.bl.y);

      } else if (type === "size") {
        return [dist(cp.tl.x, cp.tl.y, cp.tr.x, cp.tr.y),
                dist(cp.tl.x, cp.tl.y, cp.bl.x, cp.bl.y)];

      } else if (type === "left") {
        return min(cp.tl.x, cp.tr.x, cp.bl.x, cp.br.x);

      } else if (type === "right") {
        return max(cp.tl.x, cp.tr.x, cp.bl.x, cp.br.x);

      } else if (type === "top") {
        return min(cp.tl.y, cp.tr.y, cp.bl.y, cp.br.y);

      } else if (type === "bottom") {
        return max(cp.tl.y, cp.tr.y, cp.bl.y, cp.br.y);

      } else {
        // x, y or position

        var p;
        if(currRefPoint === "topLeft") {
          p = [cp.tl.x, cp.tl.y];
        } else if (currRefPoint === "center") {
          p = [(cp.tl.x + cp.br.x) / 2, (cp.tl.y + cp.br.y) / 2];
        } else if (currRefPoint === "topRight") {
          p = [cp.tr.x, cp.tr.y];
        } else if (currRefPoint === "bottomLeft") {
          p = [cp.bl.x, cp.bl.y];
        } else if (currRefPoint === "bottomRight") {
          p = [cp.br.x, cp.br.y];
        } else if (currRefPoint === "topCenter") {
          p = [(cp.tl.x + cp.tr.x) / 2, (cp.tl.y + cp.tr.y) / 2];
        } else if (currRefPoint === "bottomCenter") {
          p = [(cp.bl.x + cp.br.x) / 2, (cp.bl.y + cp.br.y) / 2];
        } else if (currRefPoint === "centerLeft") {
          p = [(cp.tl.x + cp.bl.x) / 2, (cp.tl.y + cp.bl.y) / 2];
        } else {
          p = [(cp.tr.x + cp.br.x) / 2, (cp.tr.y + cp.br.y) / 2];
        }
  ...
  } else if (item.hasOwnProperty("geometricBounds")) {
    // page item

    legalTypes = legalTypes.concat(["rotation", "scale"]);

    if(!contains(legalTypes, type)) {
      error("measure(), invalid type parameter \"" + type + "\"!\rIf measuring a page item use any of the following types: " + legalTypes.join(", ") + " or an array containing any of these types.");
    }

    if(type === "width") {
      var bounds = item.geometricBounds;
      return precision(Math.abs(bounds[3] - bounds[1]), 12);

    } else if (type === "height") {
      var bounds = item.geometricBounds;
      return precision(Math.abs(bounds[2] - bounds[0]), 12);

    } else if (type === "size") {
      var bounds = item.geometricBounds;
      return [precision(Math.abs(bounds[3] - bounds[1]), 12),
              precision(Math.abs(bounds[2] - bounds[0]), 12)];

    } else if (type === "rotation") {
      return -item.rotationAngle;

    } else if (type === "scale") {
      return [item.horizontalScale / 100, item.verticalScale / 100];

    } else {
      // position, x, y, top, left, bottom or right

      var aPoint = anchorPointToEnum(currRefPoint);
      var unitEnum = unitToEnum(currUnits);
      ...
      var pageTopLeft = refPage.resolve([AnchorPoint.TOP_LEFT_ANCHOR, BoundingBoxLimits.GEOMETRIC_PATH_BOUNDS, CoordinateSpaces.INNER_COORDINATES], CoordinateSpaces.SPREAD_COORDINATES)[0];

      pageTopLeft[0] += UnitValue(currOriginX, unitEnum).as(MeasurementUnits.POINTS);
      pageTopLeft[1] += UnitValue(currOriginY, unitEnum).as(MeasurementUnits.POINTS);

      if(type === "top" || type === "left") {

        var topLeftCoords = getCoords(AnchorPoint.TOP_LEFT_ANCHOR, CoordinateSpaces.SPREAD_COORDINATES, item, pageTopLeft, unitEnum);

        return type === "top" ? topLeftCoords[1] : topLeftCoords[0];

      } else if(type === "bottom" || type === "right") {

        var bottomRightCoords = getCoords(AnchorPoint.BOTTOM_RIGHT_ANCHOR, CoordinateSpaces.SPREAD_COORDINATES, item, pageTopLeft, unitEnum);

        return type === "bottom" ? bottomRightCoords[1] : bottomRightCoords[0];

      } else {

        var anchorPointCoords = getCoords(aPoint, CoordinateSpaces.INNER_COORDINATES, item, pageTopLeft, unitEnum);

        if(type === "x") {
          return anchorPointCoords[0];

        } else if (type === "y") {
          return anchorPointCoords[1];

        } else {
          return anchorPointCoords;
  ...
  } else if (item.hasOwnProperty("bounds")) {

    if(!contains(legalTypes, type)) {
      error("measure(), invalid type parameter \"" + type + "\"!\rIf measuring a page or spread use any of the following types: " + legalTypes.join(", ") + " or an array containing any of these types.");
    }

    // TODO

  } else {
    error("measure(), invalid first parameter! Use page item, text object, page or spread.");
  }
...
var getCornerPoints = function(pItem, unitEnum) {
  var pageTopLeft = getPageTopLeft();
  var tl = resolvePoint(AnchorPoint.TOP_LEFT_ANCHOR, pItem, pageTopLeft, unitEnum);
  var bl = resolvePoint(AnchorPoint.BOTTOM_LEFT_ANCHOR, pItem, pageTopLeft, unitEnum);
  var tr = resolvePoint(AnchorPoint.TOP_RIGHT_ANCHOR, pItem, pageTopLeft, unitEnum);
  var br = resolvePoint(AnchorPoint.BOTTOM_RIGHT_ANCHOR, pItem, pageTopLeft, unitEnum);
...
var resolvePoint = function(aPoint, item, pageTopLeft, unitEnum) {
  var coordsOnSpread = item.resolve([aPoint, BoundingBoxLimits.GEOMETRIC_PATH_BOUNDS, CoordinateSpaces.INNER_COORDINATES], CoordinateSpaces.SPREAD_COORDINATES)[0];
  var coordsOnPagePt = [coordsOnSpread[0] - pageTopLeft[0], coordsOnSpread[1] - pageTopLeft[1]];
  return [
    UnitValue(coordsOnPagePt[0], MeasurementUnits.POINTS).as(unitEnum),
    UnitValue(coordsOnPagePt[1], MeasurementUnits.POINTS).as(unitEnum)
  ]
}

var getPageTopLeft = function() {
  ...
  return refPage.resolve([AnchorPoint.TOP_LEFT_ANCHOR, BoundingBoxLimits.GEOMETRIC_PATH_BOUNDS, CoordinateSpaces.INNER_COORDINATES], CoordinateSpaces.SPREAD_COORDINATES)[0];
}


var getLineEnd = function(line, theta) {
  var rx, ry;

  if(line.words.lastItem().lines.length > 1 &&
     line !== line.words.lastItem().lines.lastItem()) {
    // last word does break, and is not the last part of the word
    // (as the only word in a line)
    var c = line.characters.lastItem();

    rx = c.endHorizontalOffset;
    ry = c.endBaseline;

    // add width of automatic hyphen
    var f = c.appliedFont.postscriptName;
    var s = line.parentTextFrames[0].absoluteHorizontalScale;

    var dist = (typeMetrics.hasOwnProperty(f) ? typeMetrics[f].hyphen : getTypeMetrics(c).hyphen) / 12 * c.pointSize * (c.horizontalScale / 100) * s / 100;

    rx += dist * cos(theta);
    ry += dist * sin(theta);

  } else if(line === line.parentStory.lines.lastItem()) {
    // end of story has no extra white space character
    rx = line.endHorizontalOffset;
    ry = line.endBaseline;
  } else {
    // regular line end with white space character
    rx = line.insertionPoints[-2].endHorizontalOffset;
    ry = line.insertionPoints[-2].endBaseline;
  }
...
var distPointLine = function(x, y, x1, y1, x2, y2) {
  return ((y2 - y1) * x - (x2 - x1) * y + x2 * y1 - y2 * x1) / dist(x1, y1, x2, y2);
}

var getTypeMetrics = function(textItem) {
  var lockedState = currentLayer().locked;
  currLayer.locked = false;
  var tf = currentDoc().textFrames.add({
    contents: "-",
  ...
  var blCap = c.baseline;

  tf.textFramePreferences.firstBaselineOffset = FirstBaseline.X_HEIGHT;
  var blX = c.baseline;

  var metrics = {
    capHeight: blCap - bl + c.ascent,
    xHeight: blX - bl + c.ascent,
    hyphen: c.endHorizontalOffset - c.horizontalOffset
  };

  tf.remove();
  currLayer.locked = lockedState;

  typeMetrics[currFont.postscriptName] = metrics;
 ...
 * @description Transforms a given page item. The type of transformation is determinded with the second parameter. The third parameter is the transformation value, either a number or an array of x and y values. The transformation's reference point (top left, bottom center etc.) can be set beforehand by using the `referencePoint()` function. There are 10 different transformation types:
...
pub.transform = function(pItem, type, value) {
  ...
  var numberError = "transform(), invalid third parameter. Use a number.";
  var arrayError = "transform(), invalid third parameter. Use an array.";
  ...
  if(type === "width") {
    ...
    if(isNumber(value)) {
      ...
      error("transform(), invalid third parameter. Use an array or a number.");
    }
...
var anchorPointToEnum = function (anchorPoint) {
  var anchorPointMap = {
  ...
  return anchorPointMap[anchorPoint];
}

var unitToEnum = function (unit) {
  var unitMap = {
    pt: MeasurementUnits.POINTS,
    mm: MeasurementUnits.MILLIMETERS,
    cm: MeasurementUnits.CENTIMETERS,
...
// resolves points for the measure and transform function
var getCoords = function(aPoint, coordSpaces, item, pageTopLeft, unitEnum) {
  var coordsOnSpread = item.resolve([aPoint, BoundingBoxLimits.GEOMETRIC_PATH_BOUNDS, coordSpaces], CoordinateSpaces.SPREAD_COORDINATES)[0];

  var coordsOnPagePt = [coordsOnSpread[0] - pageTopLeft[0], coordsOnSpread[1] - pageTopLeft[1]];

  // convert coordinates back to user units
  var coordsOnPage = [
    precision(UnitValue(coordsOnPagePt[0], MeasurementUnits.POINTS).as(unitEnum), 12),
    precision(UnitValue(coordsOnPagePt[1], MeasurementUnits.POINTS).as(unitEnum), 12)
  ];
...
// TODO remove
var transformAndMeasure = function(item, type, value) {
  ...
  // hier müssen die text-only Sachen hin, damit am Ende mit dem else statement alles abgefangen werden kann
  // if (type === "xHeight") etc.

  if(type === "width") {
    if(isNumber(value)) {
      tm = tm.scaleMatrix(value / w, 1);
      item.transform(CoordinateSpaces.PASTEBOARD_COORDINATES, aPoint, tm);
    } else {
    ...
    result = transformAndMeasure(item, "position");

  } else if (type === "rotation" || type === "rotate") {
    if(isNumber(value)) {
      tm = tm.rotateMatrix(-item.rotationAngle - value);
      item.transform(CoordinateSpaces.PASTEBOARD_COORDINATES, aPoint, tm);
    }
    result = -item.rotationAngle;
    ...
    result = [item.horizontalScale / 100, item.verticalScale / 100];

  } else if (type === "shearing"  || type === "shear") {
    if(isNumber(value)) {
      tm = tm.shearMatrix(-item.shearAngle - value);
      item.transform(CoordinateSpaces.PASTEBOARD_COORDINATES, aPoint, tm);
    }
    result = -item.shearAngle;
    ...
    var anchorPosOnSpread = item.resolve([aPoint, BoundingBoxLimits.GEOMETRIC_PATH_BOUNDS, CoordinateSpaces.INNER_COORDINATES], CoordinateSpaces.SPREAD_COORDINATES)[0];
    var anchorPosOnPagePt = [anchorPosOnSpread[0] - topLeft[0], anchorPosOnSpread[1] - topLeft[1]];
    ...
    if(type === "x") {
      if(isNumber(value)) {
        transformAndMeasure(item, "position", [value, anchorPosOnPage[1]]);
        return value;
      } else {
      ...
      if(isNumber(value)) {
        transformAndMeasure(item, "position", [anchorPosOnPage[0], value]);
        return value;
      } else {
      ...
      if(isArray(value)) {
        var offset = [value[0] - anchorPosOnPage[0], value[1] - anchorPosOnPage[1]];
        transformAndMeasure(item, "translate", offset);
        return value;
      } else {
